...
echo 当前设备名: %USERDOMAIN%
echo 当前用户名: %currentuser%
echo.
set /p tmpInput=也许会重新启动电脑。你准备好了吗? (Y/N):
...
goto :END
:START
reg delete HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Run /v AzureArcSetup /f>nul
...
echo - qWave
echo 请稍候。。。
PowerShell /Command "&{Install-WindowsFeature "Direct-Play","Wireless-Networking","qWave" -Restart}"
echo 已完成！
echo 按任意键退出后请手动重启电脑。。。
pause>nul
:END
